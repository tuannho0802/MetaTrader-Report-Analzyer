...
Since default MT5 exports often lack crucial metadata (like Magic Numbers in the ticket title), this application supports a custom 21-column CSV format generated by our MQL5 script.

### 1. CSV Format Requirements
The parser specifically looks for a header row containing exactly 21 columns. If the column count or header titles don't match, the file is rejected.

### 2. Extraction Strategy
Unlike MT4's DOM-based parsing, MT5 parsing is string-based:
- **Delimiter**: Standard comma (`,`) with double-quote (`"`) wrapping for text cells.
- **Magic Number**: Extracted directly from the second column (Index 1). This value is mapped to the `eaId` field in our unified `Trade` interface.
- **Net Profit Calculation**: The parser uses the "Net Profit" column (Index 16), which already accounts for Swap and Commission, ensuring accuracy in ROI calculations.

### 3. Unified Interface Mapping
To ensure that the `EAComparator` and `ResultsTable` work seamlessly regardless of the source, the MT5 parser maps data as follows:
- `Ticket` (MT5) → `ticket` (Trade)
- `Magic Number` (MT5) → `eaId` (Trade)
- `Comment` (MT5) → `comment` (Trade)
- `Close Time` (MT5) → `closeTime` (Trade)

### 4. Implementation Reference (`lib/mt5Parser/`)
The MT5 logic is encapsulated in a dedicated module to prevent bloating the main `parser.ts`. It leverages the same `recalculateResult` utility to ensure that filtering behavior remains identical between MT4 and MT5 sessions.
